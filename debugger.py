...
class Debugger(object):
    ...
    def __init__(self, machine, tape_list=None):
        ...
        self.wp_types = {
            messages.Types.ADD_READ_WP: armv2.WatchpointType.READ,
    ...
    def get_file(self, override=False):
        ...
        for i, (start, end) in enumerate(libs):
            ...
            section = elf._append_section(
                name,
                self.machine.mem[start:end],
                start,
                sh_flags=melf.SHF.SHF_EXECINSTR | melf.SHF.SHF_WRITE | melf.SHF.SHF_ALLOC,
            )
        ...
        for i, (value, name) in enumerate(self.symbols):
            ...
            elf.append_symbol(
                ...
                value,
                size,
                sym_binding=melf.STB.STB_GLOBAL,
                sym_type=melf.STT.STT_FUNC,
            )
    ...
    def handle_disassembly(self, message):
        ...
        dis = list(
            disassemble.Disassemble(
                self.machine, self.breakpoints, message.start, message.start + message.size, self.symbols
            )
        )
    ...
    def handle_get_regs(self, message):
        ...
        self.connection.send(
            messages.RegisterValues(self.machine.regs, self.machine.cpsr, self.machine.pc_value)
        )

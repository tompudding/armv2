...
static enum armv2_status device_operation(struct armv2 *cpu, uint32_t device_num,
                                          uint32_t arg0, uint32_t arg1, uint32_t *result)
{
...
enum armv2_status hw_manager_data_operation(struct armv2 *cpu, uint32_t crm, uint32_t aux, uint32_t crd,
...
enum armv2_status hw_manager_register_transfer(struct armv2 *cpu, uint32_t crm, uint32_t aux, uint32_t rd,
                                               uint32_t crn, uint32_t opcode)
{
    if( NULL == cpu ) {
        return ARMV2STATUS_INVALID_ARGS;
